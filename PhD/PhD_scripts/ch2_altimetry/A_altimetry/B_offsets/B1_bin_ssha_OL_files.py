...
# time_cs2 = pd.date_range('2010-11-01', '2024-12-01', freq='1MS')
#
# #------------------------------------------------------------------
# # LAND MASK
# #------------------------------------------------------------------
...
#         print(f"MERGE {files[j]} not available")  # << added
#         continue  # Skip this iteration if file is missing  # << added
#
#     # ds = xr.open_dataset(filepath)
...
for j in range(itt):
    ...
    try:
        ...
        if not all(var in ds.variables for var in required_vars) or ds.sizes == {}:
            print(f"MERGE {files[j]} is empty or missing required variables")
            continue

    except FileNotFoundError:
        print(f"MERGE {files[j]} not available")
        continue
